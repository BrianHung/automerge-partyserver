...
export const CHUNK_MAX_SIZE = 1024 * 1024;

const BATCH_SENTINEL = 'batch-sentinel';
 ...
 */
export const handleChunked = (receive: (data: MessageData) => void): MessageHandler => {
	...
	return message => {
		...
		if (isBatchSentinel(message.data)) {
			...
			// message too short to be chunked
...
function assertEquality(expected: unknown, actual: unknown, label: string) {
	if (expected !== actual) {
		throw new Error(`Mismatching ${label}! Expected ${expected}, got ${actual}`);
	} else {
