import { NetworkAdapter, PeerId, PeerMetadata, cbor } from '@automerge/automerge-repo';

import debug from 'debug';

import { assert } from './assert.js';
import { handleChunked, sendChunked } from './chunking.js';
import { FromClientMessage, FromServerMessage, JoinMessage, isErrorMessage, isPeerMessage } from './messages.js';
import { ProtocolV1 } from './protocolVersion.js';
import { toArrayBuffer } from './toArrayBuffer.js';

export class BrowserWebSocketClientAdapter extends NetworkAdapter {
	#isReady = false;
	...
	constructor(private readonly socket: WebSocket) {
		super();
		this.#log = this.#log.extend(socket.url);
	}
	...
	onMessage = handleChunked(message => this.receiveMessage(message as Uint8Array));
